describe(`TC-PBI4-1 : DECLARE-PLAN\n
    ...
    it(`[step 2] should select a 'UX - UX/UI Designer' from the dropdown and enable the 'Declare' button.`,()=>{    
    ...
    it(`[step 3] should select a '-- Select Major --' from the dropdown and disable the 'Declare' button.`,()=>{    
        cy.get('.ecors-dropdown-plan').select('-- Select Major --') ;
        cy.get('.ecors-dropdown-plan').should('contain.text','-- Select Major --'); 
        cy.get('.ecors-button-declare').should('exist').and('be.visible').and('be.disabled') ;
    })
})